...
public class Application {
	...
	private Vector<ReseauStatut> chemin;
	...
	*/
		...
		for (int l = 0; l < reseau_routier.getNombreRoutes(); l++) {			
			...
			if(rue.estUneVille()){
				...
				if(!fenetre.getPanneauControles().villeDejaPresente(ville)) {
					...
					fenetre.getPanneauInfos().ajouterVille(ville);
					fenetre.getPanneauInfos().ajouterRue(reseau_routier.getListeRoute());
				}
			}
		}
	}
	...
	public void remplirListePoints(PanelControls.jcbFlag flag) {
	...
	{
		fenetre.getPanneauInfos().chargerPoint(reseau_routier.getListPoints());
	}
	
	// Selectionne le filtre a envoyer à la comboBox
	public void selectionnerFiltre(PanelControls.jcbFlag flag) {
	...
	private void initialiserListesPoints() {
		remplirListePoints(PanelControls.jcbFlag.DEPART);
		remplirListePoints(PanelControls.jcbFlag.ARRIVEE);
	}
			...
			for (Iterator it = chemin.iterator(); it.hasNext();) {
				...
				if (numPtPrec >= 0) {
					...
					if (idEdge >= 0) {
						...
						fenetre.getPanneauInfos().ajouterRoute("Pas de route trouv\u00e9e ! (" + numPtPrec + "|" + numPt + ")");
				}
	...
	public void choixCouleur() {
		...
		fenetre.getPanneauVue().getCarte().setItineraireCouleur(Color.ORANGE);
