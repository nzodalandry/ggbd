...
public class PlusCourtChemin {
    ...
    int p1;
    int p2;
    Vector<ReseauStatut> chemin = new Vector<ReseauStatut>();
	...
	void step3() {		/* find the shortest node in Sbar */
		...
		v[u].succ = -3;
                if (v[u].id == p2) {
                  ...
                  while (true) {
                    findGoal = true;
                    chemin.insertElementAt(new ReseauStatut(v[u].id, v[u].prev), 0);
                    if (v[u].id == p1)
                      break;
                    if (v[u].prev >= 0)
                      u = v[u].prev;
	...
	public Vector<ReseauStatut> solve(int ps, int pe) {
          iteration = step = 0;
          p1 = ps;
          p2 = pe;
          findGoal = false;
          chemin.clear();
          if (p1 == p2) {
            chemin.insertElementAt(new ReseauStatut(p1, p2), 0);
            return chemin;
          }
          snode = findNode(p1);
          enode = findNode(p2);
